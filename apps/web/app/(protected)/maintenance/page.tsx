...
export default async function MaintenancePage({
  ...
  let query = supabase
    ...
    .select(
      "id, maintenance_type, service_date, cost_amount, odometer, description, vehicle_id, trip_id, vehicles(registration_number), trips(trip_code), branches(name)",
      { count: "exact" },
    )
    .is("deleted_at", null)
  ...
  if (search) {
    query = query.or(`description.ilike.%${search}%,vendor_or_workshop.ilike.%${search}%`);
  }
  ...
  if (startDate) {
    query = query.gte("service_date", `${startDate}T00:00:00`);
  }

  if (endDate) {
    query = query.lte("service_date", `${endDate}T23:59:59`);
  }

  const from = (page - 1) * PAGE_SIZE;
  const to = from + PAGE_SIZE - 1;
  const { data: maintenanceLogs, count } = await query.range(from, to);
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-6 py-12">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-semibold text-slate-900">Maintenance Logs</h1>
          <p className="mt-2 text-sm text-slate-600">
            Track maintenance expenditure and servicing.
          </p>
        </div>
          ...
          <tbody className="divide-y divide-slate-100">
            {maintenanceLogs?.length ? (
              maintenanceLogs.map((log) => {
                const vehicleReg = (log as { vehicles?: { registration_number?: string } | null })
                  .vehicles?.registration_number;
                return (
                  <tr key={log.id} className="text-slate-700">
                    <td className="px-4 py-3 font-semibold text-slate-900">{vehicleReg ?? "—"}</td>
                    <td className="px-4 py-3">{log.maintenance_type}</td>
                    <td className="px-4 py-3">{log.description}</td>
                    <td className="px-4 py-3">{log.cost_amount.toFixed(2)} SAR</td>
                    <td className="px-4 py-3">{log.odometer}</td>
                    <td className="px-4 py-3">{formatMaintenanceDateTime(log.service_date)}</td>
