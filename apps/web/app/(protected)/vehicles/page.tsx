...
export default async function VehiclesPage({
  ...
  let query = supabase
    .from("vehicles")
    .select("id, registration_number, make, model, odometer, status, branch_id", { count: "exact" })
    .is("deleted_at", null)
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-6 py-12">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm text-slate-600">Manage vehicle inventory and status.</p>
        </div>
        <Link
